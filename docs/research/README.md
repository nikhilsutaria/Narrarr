...
2. If green-lit, **run the Phase 0 validation spike** — the cheapest way to confirm the stack before real investment.
3. Then proceed through the roadmap.

> **Update (2026-06-24): a proof-of-concept was built and validated.** It proved the core loop (EPUB → offline neural voice → synced sentence highlight) on Android and pre-solved much of the neural-playback work, but on a custom reader + `audioplayers` rather than `flutter_readium` + `just_audio`.
